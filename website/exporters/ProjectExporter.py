...
class ProjectExporter(Exporter):
    ...
    def _generate_xml(self) -> str:
        ...
        self._name = name_content.text.replace(".", "_")
        ...
        for field in self._airtable.get_all_records_from_table("Field"):
            ...
            atomic_semantic_pattern = ET.SubElement(
                atomic_semantic_patterns, "atomic_semantic_pattern"
            )

            atomic_semantic_pattern_uri = ET.SubElement(atomic_semantic_pattern, "uri")
            atomic_semantic_pattern_uri.text = record.get("URI")

            atomic_semantic_pattern_name = ET.SubElement(
                atomic_semantic_pattern, "label"
            )
            atomic_semantic_pattern_name.text = record.get("UI_Name")

        composite_semantic_patterns = ET.SubElement(
            components, "composite_semantic_patterns"
        )

        composite_patterns = list(
            map(
                lambda x: {**x, **{"type": "model"}},
                self._airtable.get_all_records_from_table("Model"),
            )
        )
        composite_patterns.extend(
            list(
                map(
                    lambda x: {**x, **{"type": "collection"}},
                    self._airtable.get_all_records_from_table("Collection"),
                )
            )
        )
        ...
        for pattern in composite_patterns:
            ...
            composite_semantic_pattern = ET.SubElement(
                composite_semantic_patterns, "composite_semantic_pattern"
            )
            ...
            composite_semantic_pattern_name = ET.SubElement(
        ...
        if fields.get("Version"):
            ...
            version_number_content = ET.SubElement(
                version_number, "version_number_content"
            )
