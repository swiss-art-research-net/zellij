...
class PatternObject(object):
    ...
    def listGroupFields(self):
        ...
        return [
            x
            for x in self.patternItems[firstkey].keys()
            if not x in [PatternObject.GROUPED_DATA_FIELD]
        ]
    ...
    def listGroupFieldsTabular(self):
        ...
        return [
            x
            for x in self.patternItems[firstkey].keys()
            if not x in [PatternObject.GROUPED_DATA_FIELD, "Turtle RDF"]
        ]

    def listFields(self):
        firstkey = next(iter(self.patternItems))
        secondkey = next(
            iter(self.patternItems[firstkey][PatternObject.GROUPED_DATA_FIELD])
        )
        return [
            x
            for x in self.patternItems[firstkey][PatternObject.GROUPED_DATA_FIELD][
                secondkey
            ].keys()
            if not x in ["GroupBy"]
        ]
    ...
    def listFieldsTabular(self):
        ...
        return [
            ...
            if not x in ["GroupBy", "Turtle RDF"]
        ]
